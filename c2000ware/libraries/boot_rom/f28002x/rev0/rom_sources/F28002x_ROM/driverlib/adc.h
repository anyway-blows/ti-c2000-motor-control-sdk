...
{
    return((base == ADCA_BASE) || (base == ADCC_BASE));
}
#endif
...
//*****************************************************************************
//
//! Registers an interrupt handler for the analog-to-digital converter.
//!
//! \param base is the base address of the ADC module.
//! \param adcIntNum is interrupt number within the ADC wrapper.
//! \param handler is a pointer to the function to be called when the
//! analog-to-digital converter interrupt occurs.
//!
//! This function registers the handler to be called when an ADC interrupt
//! occurs.  This function enables the global interrupt in the interrupt
//! controller; specific ADC interrupts must be enabled via
//! ADC_enableInterrupt().  If necessary, it is the interrupt handler's
//! responsibility to clear the interrupt source via
//! ADC_clearInterruptStatus().
//!
//! \e adcIntNum can take the value \b ADC_INT_NUMBER1, \b ADC_INT_NUMBER2,
//! \b ADC_INT_NUMBER3, or \b ADC_INT_NUMBER4 to express which of the four
//! interrupts of the ADC module should be used.
//!
...
//*****************************************************************************
//
//! Unregisters an interrupt handler for the analog-to-digital converter.
//!
//! \param base is the base address of the ADC module.
//! \param adcIntNum is interrupt number within the ADC wrapper.
//!
//! This function clears the handler to be called when an ADC interrupt
...
//*****************************************************************************
//
//! Registers an event interrupt handler for the analog-to-digital converter.
//!
//! \param base is the base address of the ADC module.
//! \param handler is a pointer to the function to be called when the
//! analog-to-digital converter interrupt occurs.
//!
//! This function registers the handler to be called when an ADC event
//! interrupt occurs.  This function enables the global interrupt in the
//! interrupt controller; specific ADC interrupts must be enabled via
//! ADC_enableInterrupt().  If necessary, it is the interrupt handler's
//! responsibility to clear the interrupt source via
//! ADC_clearInterruptStatus().
//!
...
//*****************************************************************************
//
//! Unregisters an event interrupt handler for the analog-to-digital converter.
//!
//! \param base is the base address of the ADC module.
//!
//! This function clears the handler to be called when an ADC event interrupt
//! occurs.  This function also masks off the interrupt in the interrupt
//! controller so that the interrupt handler no longer is called.
//!
//! \sa Interrupt_register() for important information about registering
