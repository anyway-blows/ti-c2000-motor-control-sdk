...
//*****************************************************************************
//
//! Values that can be passed to CAN_registerInterrupt(),
//! CAN_unregisterInterrupt(), and CAN_getIntNumber() as the \e canIntNum
//! parameter.
//
//*****************************************************************************
typedef enum
{
    CAN_INT_NUMBER0,        //!< CANINT0 Interrupt
    CAN_INT_NUMBER1         //!< CANINT1 Interrupt
} CAN_IntNumber;
...
//! \param canIntNum is the interrupt line number requested
//!
//! Given a CAN controller base address and interrupt line number, returns the
//! corresponding interrupt number.
//!
//! The following are valid options for the \e canIntNum parameter:
//! - \b CAN_INT_NUMBER0  - CANINT0 Interrupt
//! - \b CAN_INT_NUMBER1  - CANINT1 Interrupt
//!
//! \return Returns a CAN interrupt number.
//
//*****************************************************************************
static uint32_t
CAN_getIntNumber(uint32_t base, CAN_IntNumber canIntNum)
{
    uint32_t intNumber;
    ...
    {
        //
        // Return the interrupt number for CAN A
        //
        case CANA_BASE:
        {
            switch(canIntNum)
            {
                case CAN_INT_NUMBER0:
                {
                    intNumber = INT_CANA0;
                    break;
                }
                case CAN_INT_NUMBER1:
                {
                    intNumber = INT_CANA1;
                    break;
                }
...
//!
//! \param base is the base address of the CAN controller.
//! \param canIntNum is the interrupt line to register.
...
//*****************************************************************************
extern void
CAN_registerInterrupt(uint32_t base, CAN_IntNumber canIntNum,
                      void (*handler)(void));
...
//! \param canIntNum is the interrupt line to un-register.
//!
...
//*****************************************************************************
extern void
CAN_unregisterInterrupt(uint32_t base, CAN_IntNumber canIntNum);
