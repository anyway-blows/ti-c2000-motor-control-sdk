...
//*****************************************************************************
static inline void
Flash_enableReadMargin0(uint32_t ctrlBase)
{
    //
    // Check the arguments.
    //
    ...
    //
    cacheEnable = HWREG(ctrlBase + FLASH_O_FRD_INTF_CTRL) &
                    FLASH_FRD_INTF_CTRL_DATA_CACHE_EN;
    ...
    //
    HWREG(ctrlBase + FLASH_O_FSPRD) &= ~(uint32_t)FLASH_FSPRD_RM1;

    //
    // Write code to enable read margin 0.
    //
    HWREG(ctrlBase + FLASH_O_FSPRD) |= FLASH_FSPRD_RM0;


    //
    // Restore the state of the cache enable bit.
    //
    HWREG(ctrlBase + FLASH_O_FRD_INTF_CTRL) |= cacheEnable;
...
//*****************************************************************************
static inline void
Flash_enableReadMargin1(uint32_t ctrlBase)
{
    //
    // Check the arguments.
    //
    ...
    //
    HWREG(ctrlBase + FLASH_O_FSPRD) &= ~(uint32_t)FLASH_FSPRD_RM0;

    //
    // Enable read margin 1.
    //
    HWREG(ctrlBase + FLASH_O_FSPRD) |= FLASH_FSPRD_RM1;
...
//*****************************************************************************
static inline void
Flash_disableReadMargin(uint32_t ctrlBase)
{
    //
    // Check the arguments.
    //
...
Flash_getStateMachineStatus(uint32_t ctrlBase)
{
    //
    // Check the arguments.
    //
    ...
    //
    return(HWREG(ctrlBase + FLASH_O_FMSTAT));
}
