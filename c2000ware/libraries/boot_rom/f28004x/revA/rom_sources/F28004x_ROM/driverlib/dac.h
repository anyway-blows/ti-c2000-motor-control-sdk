...
//!
//! The \e signal parameter must be a value from 1 to 16, where the number
//! represents the PWM signal that will be set.
//!
//! \return None.
//
//*****************************************************************************
...
//!
//! \param base is the DAC module base address
//! \param value is the code to be loaded into the active value register
//!
...
DAC_getOffsetTrim(uint32_t base)
{
    ...
    //
    value = (int16_t)(HWREGH(base + DAC_O_TRIM) & DAC_TRIM_OFFSET_TRIM_M);
    value = ((value & DAC_REG_BYTE_MASK) ^ (int16_t)0x80) - (int16_t)0x80;
...
DAC_isRegisterLocked(uint32_t base, uint16_t reg)
{
    ...
    //
    // Check the lock status on the specified registers
    //
    value = (HWREGH(base + DAC_O_LOCK) & reg);
