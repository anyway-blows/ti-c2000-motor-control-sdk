...
//
//! \internal
//! Gets the PMBUS basic interrupt number.
//!
//! \param base is the base address of the PMBUS instance used.
//!
//! Given a PMBUS base address, this function returns the corresponding
//! interrupt number.
//!
//! \return Returns an PMBUS interrupt number, or 0 if \e base is invalid.
//
//*****************************************************************************
static inline uint32_t
PMBus_getBasicIntNumber(uint32_t base)
{
    if(base == PMBUSA_BASE)
    {
        return(INT_PMBUSA);
...
PMBus_registerBasicInterrupt(uint32_t base, void (*handler)(void))
{
    ...
    //
    // Determine the interrupt number based on the PMBUS port.
    //
    intNum = PMBus_getBasicIntNumber(base);
...
//*****************************************************************************
//
// PMBus_unregisterBasicInterrupt
//
//*****************************************************************************
void PMBus_unregisterBasicInterrupt(uint32_t base)
{
    uint32_t intNum;
...
PMBus_getInterruptStatus(uint32_t base)
{
    ...
    intStatus = HWREG(base + PMBUS_O_PMBSTS) & 0x0003FFFFUL;
...
void PMBus_initSlaveMode(uint32_t base, uint16_t address, uint16_t mask)
{
    ...
    HWREG(base + PMBUS_O_PMBSC) &= (  ~PMBUS_PMBSC_SLAVE_ADDR_M &
                   ...
                   bool txPEC)
{
    ...
    while(i < nBytes)
    {
        tx_data |= (((uint32_t)*buffer & 0x000000FFUL) << (i << 3U));
        i++;
        buffer++;
    }
                 ...
                 uint16_t *buffer)
{
    ...
    if((status & PMBUS_PMBSTS_SLAVE_ADDR_READY) == 1U)
    {
                 ...
                 uint16_t *buffer)
{
    ...
    if((status & PMBUS_PMBSTS_DATA_READY) == 1U)
    {
        // Read the receive buffer
        (void)PMBus_getSlaveData(base, &buffer[0], status);
        // Compare the low byte, the address, to the arguments
...
//
//*****************************************************************************
void PMBus_generateCRCTable(uint32_t base, uint16_t *crcTable)
{
  register uint16_t i;
  register uint16_t j;
  ...
  { 
    ...
    { 
      if ( crc8_accum & 0x80U )
        crc8_accum = ( (crc8_accum << 1U) & 0xffU ) ^ 0x07U;
      else
...
//*****************************************************************************
bool 
PMBus_verifyPEC(uint32_t base, uint16_t *buffer, uint16_t *crcTable,
                uint16_t byteCount, uint16_t PEC)
{
    ...
    for(i = 0U; i < byteCount; i++)
    {
        ...
        tableIndex = accumulator ^ __byte((int *)buffer, parity);
        accumulator = crcTable[tableIndex];
        parity += 2;
    }
...
{
    // Locals
    uint32_t interruptState, clock_divider, calcModuleFreq;
    ...
    // possible value 
    clock_divider = (sysFrequency / moduleFrequency) - 1UL;
    if(clock_divider > 31UL)
    {
      clock_divider = 31UL;
    }
    // Write to the PMBCTRL register
    HWREG(base + PMBUS_O_PMBCTRL) |= 
                                  ((clock_divider << PMBUS_PMBCTRL_CLKDIV_S)
                                                   & PMBUS_PMBCTRL_CLKDIV_M);
                                                      
    // Calculate the actual bus frequency
    calcModuleFreq = sysFrequency / (clock_divider + 1UL);
                  ...
                  uint32_t moduleFrequency)
{
    ...
    timePeriod = 1000000000UL/moduleFrequency;
    ...
    switch(mode)
    {
      ...
      case PMBUS_CLOCKMODE_STANDARD:
        ...
        clockLowTimeout   = (35000000UL/timePeriod) - 1UL;
