...
PAGE 0 :
		 ...
		 FPU32_TWIDDLE_TABLE_1	: origin = 0x3F9200, length = 0x00DF8
		 RESERVED_04			: origin = 0x3F9FF8, length = 0x000208, fill = 0xFFFF /*Reserved*/
		 FPU32_TWIDDLE_TABLE_2	: origin = 0x3FA200, length = 0x000800
         ...
         FLASHAPI_IN_ROM		: origin = 0x3FEB22, length = 0x000B13
         RESERVED_05            : origin = 0x3FF635, length = 0x0008C5, fill = 0xFFFF /*Reserved*/
         FLASHAPI_APITABLE      : origin = 0x3FFEFA, length = 0x000028
...
PAGE 1 :
         ...
         FLASHAPI_EBSS			: origin = 0x760,    length = 0x000020
...
SECTIONS
{
	...
	GROUP : load = FPU32_TWIDDLE_TABLE_1, 				PAGE = 0,	ALIGN(2)
	{
		 .CFFT_f32_twiddleFactors
		 .RFFT_f32_twiddleFactors
	}

	GROUP : load = FPU32_TWIDDLE_TABLE_2, 				PAGE = 0,	ALIGN(2)
	{
		 tf512pFFTTable
	}

    FLASHAPI_IN_ROM : load = FLASHAPI_IN_ROM  	PAGE = 0,	ALIGN(2)
    {
        -lF021_API_F28004x_FPU32.lib(.text)
        -lrts2800_fpu32.lib(.text)
    }

    FLASHAPI_EBSS : load = FLASHAPI_EBSS	PAGE = 1
    {
    	-lF021_API_F28004x_FPU32.lib(.ebss)
    }

		 .romFlashApiTable: load = FLASHAPI_APITABLE,       PAGE = 0
