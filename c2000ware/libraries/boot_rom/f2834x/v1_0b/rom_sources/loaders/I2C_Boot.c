...
// Checkin $Date: May 30, 2008   14:11:56 $
//###########################################################################
//
// FILE:    I2C_Boot.c
//
...
//     - For the TIMING1 loader, the input frequency to the device must
//       be between 28Mhz and 48Mhz, creating a 7Mhz to 12Mhz system clock.
//       For the TIMING2 loader, the input frequency to the device must
//       be between 14Mhz and 24Mhz.
//       This is due to a requirement that the I2C clock be between 7Mhz and
//       12Mhz to meet all of the I2C specification timing requirements.
...
//       sending any I2C messages until the application software
...
// Private functions
inline void   I2C_Init(Uint16 Timing);
inline Uint16 I2C_CheckKeyVal(void);
inline void   I2C_ReservedFn(void);
Uint16 I2C_GetWord(void);
...
inline void I2C_Init(Uint16 Timing)
{
   ...
   if (Timing == TIMING1)           // TIMING1: 28Mhz < CLKIN < 48Mhz
   {
       I2caRegs.I2CPSC.all = 1;     // I2C clock should be between 7Mhz-12Mhz: valid for CLKIN=28-48Mhz
   }
   else                             // TIMING2: 14Mhz < CLKIN < 24Mhz
   {
       I2caRegs.I2CPSC.all = 0;     // I2C clock should be between 7Mhz-12Mhz: valid for CLKIN=14-24Mhz
