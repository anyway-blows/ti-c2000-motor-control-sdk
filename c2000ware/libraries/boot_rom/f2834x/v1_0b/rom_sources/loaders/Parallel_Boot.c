...
// Checkin $Date: May 30, 2008   14:12:17 $
//###########################################################################
//
...
Uint32 Parallel_Boot()
{
   ...
   // If an invalid key value is read, abort the load and
   // force the watchdog to reset the device.   
   if (Parallel_CheckKeyVal() == ERROR) 
   {
       WatchDogReset();
   }
