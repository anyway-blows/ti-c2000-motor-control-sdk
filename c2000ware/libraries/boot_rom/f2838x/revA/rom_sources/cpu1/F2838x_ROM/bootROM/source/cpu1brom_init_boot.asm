...
cpu1brom_version:
    .word 0x0200                                 ; CPU1 Boot ROM Version v2.0
    .word 0x0819                                 ; Month/Year: (ex: 0x0819 = 8/19 = August 2019)
...
cpu1brom_hwbist_reset_no_escape:
    ...
    MOVW    @0x2, #0x8001                        ;Update lower boot status (set boot start, HWBIST handled)
    MOVW    @0x3, #0x8000                        ;Update upper boot status (boot complete)
...
cpu1brom_perform_all_ram_inits:
    EALLOW
;
;   MemCfgRegs.DxINIT.all = 0x000F (RAMs M0, M1, D0, D1)
;
    MOVB    ACC, #0xF
    MOVW    DP, #0x17d0                          ;Set DP to MemCfgRegs
    MOVL    @0x12, ACC                           ;Write 0xF to MemCfgRegs.DxINIT.all
;
...
;
;   MemCfgRegs.GSxINIT.all = 0xFFFF (RAMS GS0 - GS15)
;
    MOVW    DP, #0x17d1                          ;Set DP to MemCfgRegs.GSxLOCK
    MOV     ACC, #0xFFFF
    MOVL    @0x12, ACC                           ;Write 0xFFFF to MemCfgRegs.GSxINIT.all
;
;   MemCfgRegs.MSGxINIT.all = 0x03FF (RAMS CPU1/CPU2 MSG0/1, CPU1/CLA1 MSG, CLA1/DMA MSG, CLA1/CPU1 MSG, CPU1/CM MSG0/1)
;
    MOV     ACC, #0x3FF
