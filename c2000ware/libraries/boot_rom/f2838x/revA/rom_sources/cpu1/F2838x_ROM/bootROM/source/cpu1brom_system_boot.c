...
//*****************************************************************************
//
// Sets up various device systems
//
// Parameters: None
//
// This function enables/configures various device systems including:
//  - System clock divider to /1
//  - Power up flash pump and banks
//  - Trim the PMM, INTOSC, APLL
//  - Configure the APLL
//  - Mask the SYS/AUX PLL Slip Interrupts
//
// Note 1: SYS/AUX PLL slip interrupts are being masked because the PLL
//         slip detection is deprecated (no longer supported) in favor of
//         using the DCC module to ensure PLL clock is correct
//
// Return: None
//
//*****************************************************************************
void CPU1BROM_setupDeviceSystems(void)
{
    ...
    //
    // Write to System Error Mask register to mask SYS/AUX PLL Slip Events
    // (The SYS/AUX PLL slip detection is no longer supported, read more
    //  in note 1 in the CPU1BROM_setupDeviceSystems() description above)
    //
    EALLOW;
    HWREG(SYSSTAT_BASE + SYSCTL_O_SYS_ERR_MASK) = (uint32_t)((HWREG(SYSSTAT_BASE + SYSCTL_O_SYS_ERR_MASK) &
                                                              SYS_ERR_MASK_LOWER_MASK) | (uint32_t)SYS_ERR_MASK_PLL_SLIP_VALUE);
    EDIS;
}
       ...
       SYSCTL_RESC_POR)
    {
        ...
        if(HWREAD_Z2_OTP_BOOT_GPREG2_KEY == GPREG2_KEY)
        {
               ...
               (HWREAD_Z2_OTP_BOOT_GPREG2_PBIST_CONFIG == GPREG2_PBIST_PLL_DISABLED))
            {
                ...
                uint16_t cmClockDivider = BROM_CMCLKCTL_CLKDIV_BY_4;
                ...
                else //if(HWREAD_Z2_OTP_BOOT_GPREG2_PBIST_CONFIG == GPREG2_PBIST_PLL_DISABLED)
                {
                    ...
                    // No PLL
                    //
                    cmClockDivider = BROM_CMCLKCTL_CLKDIV_BY_1;
                }

                //
                // Enable CM clock to run off SYSCLK
                // Use CM clock divider /4 when running with PLL enabled
                // Use CM clock divider /1 when running with INTOSC and PLL bypassed
                //
                EALLOW;
                HWREGH(CLKCFG_BASE + SYSCTL_O_CMCLKCTL) = (uint16_t)((HWREGH(CLKCFG_BASE + SYSCTL_O_CMCLKCTL) &
                                                                      ~BROM_CMCLKCTL_MASK) | (uint16_t)(BROM_CMCLKCTL_DIVSRCSEL_SYSCLK |
                                                                                                        cmClockDivider));
                EDIS;
                ...
                //
                // Set SYSCLK Divider back to /1
                //
                HWREGH(CLKCFG_BASE + SYSCTL_O_SYSCLKDIVSEL) = BROM_SYSCLKDIV_BY_1;

                //
                // Reset CMCLK Configuration back to default reset values
                //
