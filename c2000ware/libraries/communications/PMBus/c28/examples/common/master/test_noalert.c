...
// FILE:   test_noalert.c
//
// TITLE:  Alert Response to a different slave Test
//
//###########################################################################
...
{
    //
    // Important: A 2nd slave is required to be on the network to assert
    // the alert line in order to test the 1st slave software stack response.
    //
    // Lets assume 2nd slave address is 0x39
    //
    ...
    //
    PMBus_configMaster(PMBUSA_BASE, SLAVE_2_ADDRESS, 0U, PMBUS_ENABLE_WRITE);
    ...
    //
    ((pmbusMasterBuffer[0] & SLAVE_ADDRESS_MASK) == (SLAVE_2_ADDRESS << 1U)) ?
                                                    handle->pass++ :
                                                    handle->fail++;
    ...
    //
    // Do a Quick command to 1st slave
    // Config the master enable Write (by omitting the read
    // option from the configWord, you enable write), 0 byte
    //
