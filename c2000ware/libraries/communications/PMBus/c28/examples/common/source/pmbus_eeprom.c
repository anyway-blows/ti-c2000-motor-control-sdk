...
void dummyWrite(void)
{
    ...
    //
    PMBus_putMasterData(BASE, &txbuf, ONE_BYTE);
    PMBus_configMaster(BASE, SLAVE_ADDRESS, ONE_BYTE, PMBUS_WRITE);
...
uint16_t PMBusEEPROM_transmitAddress(uint16_t eepromAddress)
{
    ...
    //
    // Slave Address + PMBus_eeprom address is transmitted in WRITE mode
    //
    PMBus_configMaster(BASE, SLAVE_ADDRESS, TWO_BYTES , PMBUS_WRITE);

    PMBus_putMasterData(BASE, txbuf, TWO_BYTES);
...
uint16_t PMBusEEPROM_writeByte(uint16_t eepromAddress, uint16_t data)
{
    ...
    //
    // Slave Address + PMBus_eeprom address + databyte to be
    // programmed is transmitted in write mode
    //
    PMBus_configMaster(BASE, SLAVE_ADDRESS, totalBytes , PMBUS_WRITE);

    PMBus_putMasterData(BASE, txbuf, totalBytes);
    status = PMBusEEPROM_waitForAck();
                               ...
                               uint16_t numOfBytes)
{
    ...
    *(pbuffer + 1) = (eepromAddress & 0xFFU);
    ...
    //
    // Slave Address + PMBus_eeprom address + batch of 4 bytes of data
    ...
    {
        PMBus_putMasterData(BASE, pbuffer, FOUR_BYTES);
    ...
    if(remainingBytes)
    {
       PMBus_putMasterData(BASE, pbuffer, remainingBytes);
       status = PMBusEEPROM_waitForAck();
    }
...
uint16_t PMBusEEPROM_readCurrentAddress(uint16_t* pbuffer, byteCount_t numOfBytes)
{
    ...
    PMBus_configMaster(BASE, SLAVE_ADDRESS, numOfBytes, PMBUS_READ);
                                       ...
                                       uint16_t * pbuffer, byteCount_t numOfBytes)
{
    ...
    //
    // Transmit Slave address + EEPROM address
    //
    ...
    PMBus_getMasterData(BASE, pbuffer, FOUR_BYTES);
                                           ...
                                           uint16_t * pbuffer, uint16_t numOfBytes)
{
    ...
    //
    PMBus_configMaster(BASE, SLAVE_ADDRESS, totalBytes, PMBUS_READ);
    ...
    if(remainingBytes)
    {
       ...
       PMBus_getMasterData(BASE, pbuffer, remainingBytes);
    }
