...
//!
//! \brief  Runs the single precision Real FFT (ADC input) with windowing
...
// (51*fs/N = 51*10K/512 = 996.0937Hz) and all its odd harmonics. Since the
...
// line as follows
// "
//   OUT = FPU_DSP_runWindowedFFT('real_adc', 2,  'hann', 512, 'TYPE_FLOAT', 
//                                'FTYPE_C');
...
//  -The window needs to be re-generated for different window sizes.
//   Use the matlab script provided and generate a Hanning window for
//   the specified size.
...
// The absolute error between the result and expected values, the value is
// lowered for the phase calculation further on
float tolerance = 1.6e-5;
...
void DSP_runTest(void)
{
    ...
    while(1){
        ...
        // Calculate real FFT with windowed ADC input
...
#endif // USE_REAL_ADC_INPUT == 0U
        // Lower tolerance for the phase calculations
        tolerance = 1.5e-2;
