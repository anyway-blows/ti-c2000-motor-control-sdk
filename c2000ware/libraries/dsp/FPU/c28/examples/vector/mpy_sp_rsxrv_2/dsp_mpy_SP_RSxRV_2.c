...
// line as follows
// "
//   FPU_DSP_runVectorRoutines('mpyRSbyRV', 64, 'TYPE_FLOAT', 'FTYPE_C');
// "
...
// Also note the scalar is tacked on to the end of the real vector in the 
// input file
//
//#############################################################################
...
// The real scalar is tacked on to the end of the real vector
float *test_scalar = &test_input[TEST_SIZE];
...
void DSP_runTest(void)
{
    ...
    mpy_SP_RSxRV_2((float *)test_output, (const float *)test_input, 
                    (float)test_scalar[0U], TEST_SIZE);
