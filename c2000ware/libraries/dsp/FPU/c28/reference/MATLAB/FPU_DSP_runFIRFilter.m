...
% \brief  generate FIR Input/Ouput Vectors
...
%% Function Prototype
%  An Example:
%  R = FPU_DSP_runFIRFilter('lowpass', 1024);
%  R = FPU_DSP_runFIRFilter('lowpass', 1024, 'TYPE_FLOAT', 'FTYPE_C');
%
function R = FPU_DSP_runFIRFilter(fn, N, varargin)
%% Parse Arguments
...
filtObj = design(filtSpecObj, 'FIR');
fvtool(filtObj);
measure(filtObj)
Ni = N + length(filtObj.Numerator); %Set the input vector size
IN(N + 1 : Ni) = filtObj.Numerator;
disp(['Filter Order = ',num2str(length(filtObj.Numerator) - 1)]);
% Convolution
