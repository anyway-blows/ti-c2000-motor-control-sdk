...
% \brief  Radix 4 Inverse Fast Fourier Transform
...
%##############################################################################
%% The FFT Function
% \brief Run an inverse complex FFT (radix 4)
...
function OUT = FPU_DSP_runRadix4ICFFT(N, IN, T, TBL_SIZE)
OUT            = IN;
szofcomplex    = 2;
n_stages       = log2(N);
...
for k = 3 : 2 : n_combo_stages
    ...
    for p = 1 : n_grps
        ...
        for r = 1 : n_bflys
            ...
            Er              =  0.5*(Ar+(Br*C1+Bi*S1));
            Ei              =  0.5*(Ai+(Bi*C1-Br*S1));
            Fr              =  0.5*(Ar-(Br*C1+Bi*S1));
            Fi              =  0.5*(Ai-(Bi*C1-Br*S1));
            Gr              =  0.5*(Cr+(Dr*C1+Di*S1));
            Gi              =  0.5*(Ci+(Di*C1-Dr*S1));
            Hr              =  0.5*(Cr-(Dr*C1+Di*S1));
            Hi              =  0.5*(Ci-(Di*C1-Dr*S1));
            Ir              =  0.5*(Er+(Gr*C2+Gi*S2));
            Ii              =  0.5*(Ei+(Gi*C2-Gr*S2));
            Jr              =  0.5*(Fr-(Hr*S2-Hi*C2));
            Ji              =  0.5*(Fi-(Hi*S2+Hr*C2));
            Kr              =  0.5*(Er-(Gr*C2+Gi*S2));
            Ki              =  0.5*(Ei-(Gi*C2-Gr*S2));
            Lr              =  0.5*(Fr+(Hr*S2-Hi*C2));
            Li              =  0.5*(Fi+(Hr*C2+Hi*S2));
