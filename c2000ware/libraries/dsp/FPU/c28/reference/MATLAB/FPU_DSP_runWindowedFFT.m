...
% \brief  generate Windowed FFT Input/Ouput Vectors
...
% short input data
% * *window*, must be one of the following
% # 'barthannwin'
% # 'bartlett'
% # 'blackman'
% # 'blackmanharris'
...
%% Function Prototype
%  An Example:
%  OUT = FPU_DSP_runWindowedFFT('complex', 2, 'hann', 1024);
%  OUT = FPU_DSP_runWindowedFFT('real', 4,  'hann', 1024, 'TYPE_FLOAT',
%                               'FTYPE_C');
%
function OUT = FPU_DSP_runWindowedFFT(ffttype, radix, window, N, varargin)
%% Parse Arguments
close all
clc
...
assert(strcmp(window, 'barthannwin')       || ...
    strcmp(window, 'bartlett')          || ...
    strcmp(window, 'blackman')          || ...
    strcmp(window, 'blackmanharris')    || ...
    strcmp(window, 'bohmanwin')         || ...
    strcmp(window, 'chebwin')           || ...
    strcmp(window, 'flattopwin')        || ...
    strcmp(window, 'gausswin')          || ...
    strcmp(window, 'hamming')           || ...
    strcmp(window, 'hann')              || ...
    strcmp(window, 'kaiser')            || ...
    strcmp(window, 'nuttallwin')        || ...
    strcmp(window, 'parzenwin')         || ...
    strcmp(window, 'rectwin')           || ...
    strcmp(window, 'taylorwin')         || ...
    strcmp(window, 'triang')            || ...
    strcmp(window, 'tukeywin'),...
    estr);
...
if(strcmp(filetype, 'FTYPE_C'))
    TblFn = 'fft_tables.c';
    WdwFn = 'fft_windowTbl.c';
    InFn  = strcat(strcat('w',ffttype(1),'fft',num2str(radix)),'Input.c');
    OutFn = strcat(strcat('w',ffttype(1),'fft',num2str(radix)),'Output.c');
    ext = '.c';
else % FTYPE_ASM
    TblFn = 'fft_tables.asm';
    WdwFn = 'fft_windowTbl.asm';
    InFn  = strcat(strcat('w',ffttype(1),'fft',num2str(radix)),'Input.asm');
    OutFn = strcat(strcat('w',ffttype(1),'fft',num2str(radix)),'Output.asm');
    ext = '.asm';
end
...
%% Generate the Window (size = N)
WINDOW_SIZE  = N;
ADC_BITS  = 12;
W = FPU_DSP_genWindowFFT(WINDOW_SIZE, window, precision, WdwFn);
...
Ampy = randi([35 (2^ADC_BITS-1)/n_sines], n_sines, 1);
...
% Also make a copy of the input before windowing (debugging)
if strcmp(ffttype,'complex')
    IN2(1:2:2*N-1,1) = real(IN);
    IN2(2:2:2*N,1)   = imag(IN);
else %strcmp(ffttype,'real') || strcmp(ffttype,'real_alt') || 
     %strcmp(ffttype, 'real_adc')
    IN2 = IN;
end
...
% Note if IN is real, the fft will take the
% imaginary points to be 0. You do not have to
% manually add in zeros
if(strcmp(ffttype,'real_adc'))
    IN    = double(floor(IN + abs(min(IN))));
    IN2   = double(floor(IN2 + abs(min(IN2))));
    W     = double(floor(W.*(2^16))); %conver to Q16
    % Window the input
    IN    = double(floor((IN .* W)./(2^16)));
...
figure(1),
subplot(4,2,1), plot(real(IN)), legend('Windowed Input Signal(real)');
subplot(4,2,2), plot(imag(IN)), legend('Windowed Input Signal(imag)');
subplot(4,2,3), plot(abs(TEMP)), legend('Windowed FFT(MATLAB)');
clear TEMP i;
...
figure(1),
subplot(4,2,4), plot(abs(temp)), legend('Windowed FFT(Routine)');
