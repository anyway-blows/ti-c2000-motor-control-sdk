;;#############################################################################
;;! \file source/fpu64/fft/rfft_f64.asm
;;!
;;! \brief  Real Fast Fourier Transform (Double Precision)
...
;;| DATE     | AUTHOR        | CHANGE                                         |
;;+----------+---------------+------------------------------------------------+
;;| 05/03/19 | Ritvik Sadana | Replaced RFFT_f64_calc with RFFT_f64 to        |
...
;;  a multiple (x4) of the FFT size. If it is not aligned then the output 
;;  buffer will yield invalid results. If you do not wish to align the input 
;;  buffer then use the RFFT_f64u function. Using this function will 
;;  reduce cycle performance of the algorithm.
;;
;; FUNCTIONS:
;;  void RFFT_f64 (CFFT_F64_Handle)
;;  
...
;;
    .if __TI_EABI__
    .asg    ".bss", .ebss
    .asg    RFFTF64_vars, _RFFTF64_vars
    .endif 
_RFFTF64_vars_InPtr             .usect .ebss,2,1,1
_RFFTF64_vars_OutPtr            .usect .ebss,2,1,1
_RFFTF64_vars_TwiddlesPtr       .usect .ebss,2,1,1
_RFFTF64_vars_Stages            .usect .ebss,1,1,0
_RFFTF64_vars_FFTSize           .usect .ebss,1,1,0
...
_RFFT_f64:
    ...
    .asg    @_RFFTF64_vars_InPtr           , STAGE_1_2_3_INPUT_PTR
    .asg    @_RFFTF64_vars_OutPtr          , STAGE_1_2_3_OUTPUT_PTR
    .asg    @_RFFTF64_vars_TwiddlesPtr     , STAGE_4UP_TWIDDLES_PTR
    .asg    @_RFFTF64_vars_Stages          , FFT_STAGES
    .asg    @_RFFTF64_vars_FFTSize         , FFT_SIZE
;;-----------------------------------------------------------------------------
;; Context Save
;;-----------------------------------------------------------------------------
    PUSH    XAR1
...
;;-----------------------------------------------------------------------------
    ...
    LCR     _RFFT_f64_stages1and2and3withBitReverse
    LCR     _RFFT_f64_stages4andUp
...
;; - Input buffer must be aligned to a 4N word boundary
;;
;; Register Usage:                                                            
;;   XAR0 : 2*FFT size / output pointer increment                
;;   XAR1 : 2*FFT size (copy)
...
;;*****************************************************************************
    .page
    .sect   ".text"
_RFFT_f64_stages1and2and3withBitReverse:
    .asmfunc
    .asg    @_RFFTF64_vars_InPtr        , STAGE_1_2_3_INPUT_PTR
    .asg    @_RFFTF64_vars_OutPtr       , STAGE_1_2_3_OUTPUT_PTR
    .asg    @_RFFTF64_vars_FFTSize      , FFT_SIZE
    .asg    @_RFFTF64_vars_TwiddlesPtr  , STAGE_1_2_3_TWIDDLES_PTR
...
;;-----------------------------------------------------------------------------
    ...
    MOVZ    AR1, AL                         ; XAR1 = [0:2*FFT size] (copy)
    LSR     AL, #4
    SUBB    AL, #1                          ; AL = (FFT size/8)-1
    .align  2
    RPTB    _RFFT_f64_stages1and2and3withBitReverse_loop, AL
;;   +-----+----------------------------------------------------+
...
;;   +-----+-----+----------------------------------------------+
    MOV32   R0H, *+XAR2[2]      ;  | | | R0H = D0.H,                  ARP->XAR2
    MOV32   R0L, *BR0++         ;  | | | R0  = D0
    MOV32   R1H, *+XAR2[2]      ;  | | | R1H = D1.H
    MOV32   R1L, *BR0++         ;  | | | R1  = D1
    ADDF64  R2, R0, R1          ; *| | | R2  = D0+D1
 || MOV32   R3H, *+XAR2[2]      ;  | | | R3H = D2.H
    MOV32   R3L, *BR0++         ; 1| | | R3  = D2
    SUBF64  R0, R0, R1          ; 2|*| | R0  = D0-D1
 || MOV32   R1H, *+XAR2[2]      ;  | | | R1H = D3.H
    MOV32   R1L, *BR0++         ;  |1| | R1  = D3
    ADDF64  R4, R3, R1          ; *|2| | R4  = D2+D3 
 || MOV32   R5H, *+XAR2[2]      ;  | | | R5H = D4.H
    MOV32   R5L, *BR0++         ; 1| | | R5  = D4
    SUBF64  R1, R1, R3          ; 2|*| | R1  = D3-D2
 || MOV32   R3H, *+XAR2[2]      ;  | | | R3H = D5.H
    MOV32   R3L, *BR0++         ;  |1| | R3  = D5
    ADDF64  R6, R5, R3          ; *|2| | R6  = D4+D5
 || MOV32   R7H, *+XAR2[2]      ;  | | | R7H = D6.H
    MOV32   R7L, *BR0++         ; 1| | | R7  = D6
    SUBF64  R5, R5, R3          ; 2|*| | R5  = D4-D5
    SUBF64  R3, R2, R4          ; *|1| | R3  = (D0+D1)-(D2+D3)
    ADDF64  R2, R2, R4          ; 1|2|*| R2  = (D0+D1)+(D2+D3)
 || MOV32   R4H, *+XAR2[2]      ;  | | | R4H = D7.H
    MOV32   R4L, *BR0++         ; 2| |1| R4  = D7
 ...
 || MOV32   *XAR3++, R5L        ;  | | | save R5L,                    ARP->XAR3
    ...
    MOVZ    AR0, AR1            ;  | | | XAR0 = [0:2*FFT size]
_RFFT_f64_stages1and2and3withBitReverse_loop:
    LRETR
    .endasmfunc
    .unasg  STAGE_1_2_3_INPUT_PTR
    .unasg  STAGE_1_2_3_OUTPUT_PTR
...
_RFFT_f64_stages4andUp:
    ...
    .asg    XAR7,                           COSptr
    .asg    XAR7,                           SINptr
    .asg    @_RFFTF64_vars_InPtr,           STAGE_4_UP_INPUT_PTR    
    .asg    @_RFFTF64_vars_OutPtr,          STAGE_4_UP_OUTPUT_PTR    
    .asg    @_RFFTF64_vars_TwiddlesPtr,     STAGE_4_UP_TWIDDLES_PTR
    .asg    @_RFFTF64_vars_Stages,          STAGE_4_UP_FFT_STAGES     
    .asg    @_RFFTF64_vars_FFTSize,         STAGE_4_UP_FFT_SIZE
...
;;-----------------------------------------------------------------------------
    MOVW    DP, #_RFFTF64_vars_InPtr    ; Load the DP once
    MOV     AL, STAGE_4_UP_FFT_STAGES
    SUB     AL, #3                      ; If (fft->FFTStages <= 3) exit
    B       _RFFT_f64_stages4andUp_end, LEQ
...
;; Stage 4 and Up    
;;-----------------------------------------------------------------------------
_RFFT_f64_stages4andUp_stageLoop:
;;-----------------------------------------------------------------------------
;;
;; NOTE: Instead of referring to the real part as Rn we choose Dn to avoid 
;; confusing them with the Rn FPU registers
...
;;      *X4ptr = Y4 = -X4 (*X4ptr)
;;-----------------------------------------------------------------------------
_RFFT_f64_stages4andUp_outerLoop:
 ...
 || MOV32   *X3ptr++, R4L           ;  | | | 
    ...
    .align  2
    RPTB    _RFFT_f64_stages4andUp_innerLoop, InnerLoopCnt
...
_RFFT_f64_stages4andUp_innerLoop:
 ...
 || MOV32   *+X4ptr[0], R7L         ;  | | |
    ...
    BANZ    _RFFT_f64_stages4andUp_outerLoop, OuterLoopCnt--
;------------------------------------------------------------------------------
;   End of Outer Loop
...
;------------------------------------------------------------------------------
    ...
    DEC     *-SP[IDX_I]             ; IDX_I--
    BF      _RFFT_f64_stages4andUp_stageLoop, NEQ

_RFFT_f64_stages4andUp_end:
    SUBB    SP, #FRMSZ
    LRETR
    .endasmfunc
    .unasg  X3ptr
