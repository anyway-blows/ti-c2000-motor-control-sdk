;;#############################################################################
;;! \file source/fpu64/fft/rfft_f64_adc.asm
;;!
;;! \brief  Real Fast Fourier Transform (Double Precision) with ADC input
...
;;| DATE     | AUTHOR        | CHANGE                                         |
;;+----------+---------------+------------------------------------------------+
;;| 05/03/19 | Ritvik Sadana | Replaced RFFT_f64_adc_calc with RFFT_adc_f64   |
...
;;  the FFT size. If it is not aligned then the output buffer will yield 
;;  invalid results. If you do not wish to align the input buffer then use the 
;;  RFFT_adc_f64u function. Using this function will reduce cycle
;;  performance of the algorithm.
;;
;; FUNCTIONS:
;;  void RFFT_adc_f64 (CFFT_ADC_f64_Handle)
;;  
...
;;
    .if __TI_EABI__
    .asg    ".bss", .ebss
    .asg    RFFTF64_ADC_vars, _RFFTF64_ADC_vars
    .endif 
_RFFTF64_ADC_vars_InPtr             .usect .ebss,2,1,1
_RFFTF64_ADC_vars_OutPtr            .usect .ebss,2,1,1
_RFFTF64_ADC_vars_TwiddlesPtr       .usect .ebss,2,1,1
_RFFTF64_ADC_vars_Stages            .usect .ebss,1,1,0
_RFFTF64_ADC_vars_FFTSize           .usect .ebss,1,1,0


;;*****************************************************************************
;; void RFFT_adc_f64(CFFT_f64_Handle)
;;
...
;;*****************************************************************************
    .if __TI_EABI__
    .asg    RFFT_adc_f64, _RFFT_adc_f64
    .endif
    
    .page
    .global _RFFT_adc_f64
    .sect   ".text"
_RFFT_adc_f64:
    .asmfunc
    .asg    #2                             , LF_SIZE    
    ...
    .asg    @_RFFTF64_ADC_vars_InPtr       , STAGE_1_2_3_INPUT_PTR
    .asg    @_RFFTF64_ADC_vars_OutPtr      , STAGE_1_2_3_OUTPUT_PTR
    .asg    @_RFFTF64_ADC_vars_TwiddlesPtr , STAGE_4UP_TWIDDLES_PTR
    .asg    @_RFFTF64_ADC_vars_Stages      , FFT_STAGES
    .asg    @_RFFTF64_ADC_vars_FFTSize     , FFT_SIZE
;;-----------------------------------------------------------------------------
;; Context Save
;;-----------------------------------------------------------------------------
    PUSH    XAR1
...
;;-----------------------------------------------------------------------------
    ...
    ; single page
    MOVW    DP, #_RFFTF64_ADC_vars_InPtr
    ...
    LCR     _RFFT_adc_f64_stages1and2and3withBitReverse
    LCR     _RFFT_adc_f64_stages4andUp
...
;;*****************************************************************************
;; void RFFT_adc_f64_stages1and2and3withBitReverse(void)
;;
...
;; - Input buffer must be aligned to a N word boundary
;;
;; Register Usage:                                                            
...
;;*****************************************************************************
    .page
    .sect   ".text"
_RFFT_adc_f64_stages1and2and3withBitReverse:
    .asmfunc
    .asg    @_RFFTF64_ADC_vars_InPtr        , STAGE_1_2_3_INPUT_PTR
    .asg    @_RFFTF64_ADC_vars_OutPtr       , STAGE_1_2_3_OUTPUT_PTR
    .asg    @_RFFTF64_ADC_vars_FFTSize      , FFT_SIZE
    .asg    @_RFFTF64_ADC_vars_TwiddlesPtr  , STAGE_1_2_3_TWIDDLES_PTR
    .asg    *-SP[2]                         , STK_D7_H 
    .asg    *-SP[4]                         , STK_D7_L 
    .asg    *-SP[6]                         , STK_D6_H 
    .asg    *-SP[8]                         , STK_D6_L 
...
;;-----------------------------------------------------------------------------
    ...
    LSR     AH, #2
    SUBB    AH, #1                          ; AH  = (FFT size/8)-1
    MOVZ	AR1, AH                         ; AR1 = (FFT size/8)-1

    .align  2
_RFFT_adc_f64_stages1and2and3withBitReverse_loop:
;------------------------------------------------------------------------------
;  Input buffer must be aligned for this code
;------------------------------------------------------------------------------
    NOP         *, ARP2             ;                                 ARP->XAR2
    MOVI32      R4H, 0x39800801     ; scale factor of 1/(2^12 - 1)
    UI16TOF32   R0H, *BR0++         ; *|   D0 load
    UI16TOF32   R1H, *BR0++         ; 1|*| D1 load
    MPYF32      R0H, R0H, R4H       ; *|1| D0 scale 
    UI16TOF32   R2H, *BR0++         ; 1|*| D2 load
    F32TOF64    R0, R0H             ;  |1| 
    MPYF32      R1H, R1H, R4H       ; *| | D1 scale
 || MOV32       STK_D0_H, R0H       ;  | | D0 save
    MOV32       STK_D0_L, R0L       ; 1| |
    F32TOF64    R1, R1H             ;  | |
    UI16TOF32   R0H, *BR0++         ; *| | D3 load
    MPYF32      R2H, R2H, R4H       ; 1|*| D1 scale
 || MOV32       STK_D1_H, R1H       ;  | | D0 save
    MOV32       STK_D1_L, R1L       ;  |1|
    F32TOF64	R2, R2H				;  | |
    UI16TOF32   R1H, *BR0++         ; *| | D4 load       
    MPYF32      R0H, R0H, R4H       ; 1|*| D3 scale
 || MOV32       STK_D2_H, R2H       ;  | | D2 save
    MOV32       STK_D2_L, R2L       ;  |1|
	F32TOF64    R0, R0H             ;  | |
    UI16TOF32   R2H, *BR0++         ; *| | D5 load 
    MPYF32      R1H, R1H, R4H       ; 1|*| D4 scale
 || MOV32       STK_D3_H, R0H       ;  | | D3 save
    MOV32       STK_D3_L, R0L       ;  |1|
	F32TOF64    R1, R1H             ;  | |
    UI16TOF32   R0H, *BR0++         ; *| | D6 load 
    MPYF32      R2H, R2H, R4H       ; 1|*| D5 scale
 || MOV32       STK_D4_H, R1H       ;  | | D4 save
    MOV32       STK_D4_L, R1L       ;  |1|
    F32TOF64	R2, R2H				;  | |
    UI16TOF32   R1H, *BR0++         ; *| | D7 load 
    MPYF32      R0H, R0H, R4H       ; 1|*| D6 scale
...
;;   +-----+-----+----------------------------------------------+
 ...
 || MOV32   *XAR3++, R5L        ;  | | | save R5L,                    ARP->XAR3
	...
	BANZ	_RFFT_adc_f64_stages1and2and3withBitReverse_loop, AR1--
	...
	.unasg	LF_SIZE
;;*****************************************************************************
;; void RFFT_adc_f64_stages4andUp(void)
;;
;; Stages 4 and up
;;
;; NOTE: 
...
_RFFT_adc_f64_stages4andUp:
    ...
    .asg    XAR7                            , COSptr
    .asg    XAR7                            , SINptr
    .asg    @_RFFTF64_ADC_vars_InPtr        , STAGE_4_UP_INPUT_PTR    
    .asg    @_RFFTF64_ADC_vars_OutPtr       , STAGE_4_UP_OUTPUT_PTR    
    .asg    @_RFFTF64_ADC_vars_TwiddlesPtr  , STAGE_4_UP_TWIDDLES_PTR
    .asg    @_RFFTF64_ADC_vars_Stages       , STAGE_4_UP_FFT_STAGES     
    .asg    @_RFFTF64_ADC_vars_FFTSize      , STAGE_4_UP_FFT_SIZE
...
;;-----------------------------------------------------------------------------
    MOVW    DP, #_RFFTF64_ADC_vars_InPtr ; Load the DP once
    MOV     AL, STAGE_4_UP_FFT_STAGES
    SUB     AL, #3                       ; If (fft->FFTStages <= 3) exit
    B       _RFFT_adc_f64_stages4andUp_end, LEQ
...
;; Stage 4 and Up    
;;-----------------------------------------------------------------------------
_RFFT_adc_f64_stages4andUp_stageLoop:
;;-----------------------------------------------------------------------------
;;
;; NOTE: Instead of referring to the real part as Rn we choose Dn to avoid 
;; confusing them with the Rn FPU registers
...
;;      *X4ptr = Y4 = -X4 (*X4ptr)
;;-----------------------------------------------------------------------------
_RFFT_adc_f64_stages4andUp_outerLoop:
 ...
 || MOV32   *X3ptr++, R4L           ;  | | | 
    ...
    .align  2
    RPTB    _RFFT_adc_f64_stages4andUp_innerLoop, InnerLoopCnt
...
_RFFT_adc_f64_stages4andUp_innerLoop:
 ...
 || MOV32   *+X4ptr[0], R7L         ;  | | |
    ...
    BANZ    _RFFT_adc_f64_stages4andUp_outerLoop, OuterLoopCnt--
;------------------------------------------------------------------------------
;   End of Outer Loop
...
;------------------------------------------------------------------------------
    ...
    DEC     *-SP[IDX_I]             ; IDX_I--
    BF      _RFFT_adc_f64_stages4andUp_stageLoop, NEQ

_RFFT_adc_f64_stages4andUp_end:
    SUBB    SP, #FRMSZ
    LRETR
    .endasmfunc
    .unasg  X3ptr
