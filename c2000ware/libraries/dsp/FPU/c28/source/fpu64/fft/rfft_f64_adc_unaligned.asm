;;#############################################################################
;;! \file source/fpu64/fft/rfft_f64_adc_unaligned.asm
;;!
;;! \brief  Unaligned Real Fast Fourier Transform (Double Precision) with ADC 
;;!         input
...
;;| DATE     | AUTHOR        | CHANGE                                         |
;;+----------+---------------+------------------------------------------------+
;;| 05/03/19 | Ritvik Sadana | Replaced RFFT_f64_u_adc_calc with RFFT_adc_f64u|
;;|          |               | to keep function name consistent with fpu32.   |
;;+----------+---------------+------------------------------------------------+
;;
;; DESCRIPTION:
;;
;;  This function computes a real FFT.  The input buffer needn't be aligned to
;;  the FFT size. If it is possible to align the input to a boundary equal to 
;;  the FFT size then use the faster RFFT_adc_f64 instead
;;
;; FUNCTIONS:
;;  void RFFT_adc_f64u (CFFT_ADC_f64_Handle)
;;  
;;   where CFFT_ADC_f64_Handle is defined as:
;;   typedef CFFT_ADC_f64_Struct* CFFT_ADC_f64_Handle;
;;  
;;  and where CFFT_ADC_f64_Struct and CFFT_f64_Struct are structures defined as:
;;  
;;  typedef struct {
...
;;    void     *p_tail;          //!< Null pointer to the p_output of CFFT_f64
;;  } CFFT_ADC_f64_Struct;
;;
;; ASSUMPTIONS:
;;
...
;;
    .if __TI_EABI__
    .asg    ".bss", .ebss
    .asg    RFFTF64_U_ADC_vars, _RFFTF64_U_ADC_vars
    .endif 
_RFFTF64_U_ADC_vars_InPtr             .usect .ebss,2,1,1
_RFFTF64_U_ADC_vars_OutPtr            .usect .ebss,2,1,1
_RFFTF64_U_ADC_vars_TwiddlesPtr       .usect .ebss,2,1,1
_RFFTF64_U_ADC_vars_Stages            .usect .ebss,1,1,0
_RFFTF64_U_ADC_vars_FFTSize           .usect .ebss,1,1,0


;;*****************************************************************************
;; void RFFT_adc_f64u(CFFT_f64_Handle)
;;
...
;;*****************************************************************************
    .if __TI_EABI__
    .asg    RFFT_adc_f64u, _RFFT_adc_f64u
    .endif
    
    .page
    .global _RFFT_adc_f64u
    .sect   ".text"
_RFFT_adc_f64u:
    .asmfunc
    .asg    #2                               , LF_SIZE    
    ...
    .asg    @_RFFTF64_U_ADC_vars_InPtr       , STAGE_1_2_3_INPUT_PTR
    .asg    @_RFFTF64_U_ADC_vars_OutPtr      , STAGE_1_2_3_OUTPUT_PTR
    .asg    @_RFFTF64_U_ADC_vars_TwiddlesPtr , STAGE_4UP_TWIDDLES_PTR
    .asg    @_RFFTF64_U_ADC_vars_Stages      , FFT_STAGES
    .asg    @_RFFTF64_U_ADC_vars_FFTSize     , FFT_SIZE
;;-----------------------------------------------------------------------------
;; Context Save
;;-----------------------------------------------------------------------------
    PUSH    XAR1
...
;;-----------------------------------------------------------------------------
    ...
    ; single page
    MOVW    DP, #_RFFTF64_U_ADC_vars_InPtr
    MOVL    STAGE_1_2_3_INPUT_PTR, XAR0
    MOVL	XAR4, *+XAR4[0]		      ; XAR4 = obj.p_tail
    ...
    LCR     _RFFT_adc_f64u_stages1and2and3withBitReverse
    LCR     _RFFT_adc_f64u_stages4andUp
...
;;*****************************************************************************
;; void RFFT_adc_f64u_stages1and2and3withBitReverse(void)
;;
...
;;   |_______|<- D0_H                         (SP-30)          |
;;   |_______|<- D0_L                         (SP-32)----------
;;   |_______|<- rpc calling function         (SP-34)
;;
;;*****************************************************************************
    .page
    .sect   ".text"
_RFFT_adc_f64u_stages1and2and3withBitReverse:
    .asmfunc
    .asg    @_RFFTF64_U_ADC_vars_InPtr        , STAGE_1_2_3_INPUT_PTR
    .asg    @_RFFTF64_U_ADC_vars_OutPtr       , STAGE_1_2_3_OUTPUT_PTR
    .asg    @_RFFTF64_U_ADC_vars_FFTSize      , FFT_SIZE
    .asg    @_RFFTF64_U_ADC_vars_TwiddlesPtr  , STAGE_1_2_3_TWIDDLES_PTR
    .asg    *-SP[2]                           , STK_D7_H 
    .asg    *-SP[4]                           , STK_D7_L 
    .asg    *-SP[6]                           , STK_D6_H 
    .asg    *-SP[8]                           , STK_D6_L 
    ...
    ADDB	SP, #LF_SIZE				; Create Local Frame
;;-----------------------------------------------------------------------------
;; Stages 1, 2 and 3
;;
...
;;-----------------------------------------------------------------------------
    ...
    MOV     ACC, FFT_SIZE << #15            ; ACC = FFT size / 2
    MOVZ    AR0, AH                         ; XAR0 = [0:FFT size/2]
    MOV     AL, AH                          ; AL   = [0:FFT size/2] (copy)
    LSR     AH, #2                          ; AH  = (FFT size/8)
    MOVL    XAR1, #0x0000                   ; start with an index of 0

    .align  2
_RFFT_adc_f64u_stages1and2and3withBitReverse_loop:
;------------------------------------------------------------------------------
;  Input buffer must be aligned for this code
;------------------------------------------------------------------------------
    MOVI32      R4H, 0x39800801     ; scale factor of 1/(2^12 - 1)
    UI16TOF32   R0H, *+XAR2[AR1]    ; *|   D0 load
    NOP         *, ARP1             ; 1|                              ARP->XAR1
    NOP         *BR0++              ;  |                      XAR1 <rcadd>= AR0
    UI16TOF32   R1H, *+XAR2[AR1]    ; *|   D1 load
    NOP         *, ARP1             ; 1|                              ARP->XAR1
    NOP         *BR0++              ;  |                      XAR1 <rcadd>= AR0
    MPYF32      R0H, R0H, R4H       ; *|   D0 scale 
    UI16TOF32   R2H, *+XAR2[AR1]    ; 1|*| D2 load
    NOP         *, ARP1             ;  |1|                            ARP->XAR1
    NOP         *BR0++              ;  |                      XAR1 <rcadd>= AR0
    F32TOF64    R0, R0H             ;  | 
    MPYF32      R1H, R1H, R4H       ; *|   D1 scale
 || MOV32       STK_D0_H, R0H       ;  |   D0 save
    MOV32       STK_D0_L, R0L       ; 1|   
    F32TOF64    R1, R1H             ;  |   
    UI16TOF32   R0H, *+XAR2[AR1]    ; *|   D3 load
    NOP         *, ARP1             ; 1|                              ARP->XAR1
    NOP         *BR0++              ;  |                      XAR1 <rcadd>= AR0
    MPYF32      R2H, R2H, R4H       ; *|   D1 scale
 || MOV32       STK_D1_H, R1H       ;  |   D0 save
    MOV32       STK_D1_L, R1L       ; 1|   
    F32TOF64	R2, R2H				;  |   
    UI16TOF32   R1H, *+XAR2[AR1]    ; *|   D4 load
    NOP         *, ARP1             ; 1|                              ARP->XAR1
    NOP         *BR0++              ;  |                      XAR1 <rcadd>= AR0     
    MPYF32      R0H, R0H, R4H       ; *|   D3 scale
 || MOV32       STK_D2_H, R2H       ;  |   D2 save
    MOV32       STK_D2_L, R2L       ; 1|
	F32TOF64    R0, R0H             ;  |
    UI16TOF32   R2H, *+XAR2[AR1]    ; *|   D5 load
    NOP         *, ARP1             ; 1|                              ARP->XAR1
    NOP         *BR0++              ;  |                      XAR1 <rcadd>= AR0
    MPYF32      R1H, R1H, R4H       ; *|   D4 scale
 || MOV32       STK_D3_H, R0H       ;  |   D3 save
    MOV32       STK_D3_L, R0L       ; 1|
	F32TOF64    R1, R1H             ;  |
    UI16TOF32   R0H, *+XAR2[AR1]    ; *|   D6 load
    NOP         *, ARP1             ; 1|                              ARP->XAR1
    NOP         *BR0++              ;  |                      XAR1 <rcadd>= AR0
    MPYF32      R2H, R2H, R4H       ; *|   D5 scale
 || MOV32       STK_D4_H, R1H       ;  |   D4 save
    MOV32       STK_D4_L, R1L       ; 1|
    F32TOF64	R2, R2H				;  |
    UI16TOF32   R1H, *+XAR2[AR1]    ; *|   D7 load
    NOP         *, ARP1             ; 1|                              ARP->XAR1
    NOP         *BR0++              ;  |                      XAR1 <rcadd>= AR0
    MPYF32      R0H, R0H, R4H       ; *|   D6 scale
 || MOV32       STK_D5_H, R2H       ;  |   D5 save
    MOV32       STK_D5_L, R2L       ; 1|
	F32TOF64    R0, R0H             ;  |  
    MPYF32      R1H, R1H, R4H       ; *|   D7 scale            
 || MOV32       STK_D6_H, R0H       ;  |   D6 save
    MOV32       STK_D6_L, R0L       ; 1|  
	F32TOF64    R1, R1H             ;  |  
    MOV32       STK_D7_H, R1H       ;  |   D7 save
    MOV32       STK_D7_L, R1L       ;  |  
...
;;   +-----+-----+----------------------------------------------+
    MOV32   R0H, STK_D0_H       ;  | | | R0H = D0.H,                  ARP->XAR2
    MOV32   R0L, STK_D0_L       ;  | | | R0  = D0
    MOV32   R1H, STK_D1_H       ;  | | | R1H = D1.H
    MOV32   R1L, STK_D1_L       ;  | | | R1  = D1
    ADDF64  R2, R0, R1          ; *| | | R2  = D0+D1
 || MOV32   R3H, STK_D2_H       ;  | | | R3H = D2.H
    MOV32   R3L, STK_D2_L       ; 1| | | R3  = D2
    SUBF64  R0, R0, R1          ; 2|*| | R0  = D0-D1
 || MOV32   R1H, STK_D3_H       ;  | | | R1H = D3.H
    MOV32   R1L, STK_D3_L       ;  |1| | R1  = D3
    ADDF64  R4, R3, R1          ; *|2| | R4  = D2+D3 
 || MOV32   R5H, STK_D4_H       ;  | | | R5H = D4.H
    MOV32   R5L, STK_D4_L       ; 1| | | R5  = D4
    SUBF64  R1, R1, R3          ; 2|*| | R1  = D3-D2
 || MOV32   R3H, STK_D5_H       ;  | | | R3H = D5.H
    MOV32   R3L, STK_D5_L       ;  |1| | R3  = D5
    ADDF64  R6, R5, R3          ; *|2| | R6  = D4+D5
 || MOV32   R7H, STK_D6_H       ;  | | | R7H = D6.H
    MOV32   R7L, STK_D6_L       ; 1| | | R7  = D6
    SUBF64  R5, R5, R3          ; 2|*| | R5  = D4-D5
    SUBF64  R3, R2, R4          ; *|1| | R3  = (D0+D1)-(D2+D3)
    ADDF64  R2, R2, R4          ; 1|2|*| R2  = (D0+D1)+(D2+D3)
 || MOV32   R4H, STK_D7_H       ;  | | | R4H = D7.H
    MOV32   R4L, STK_D7_L       ; 2| |1| R4  = D7
    MOV32   *XAR3++, R3L        ;  | |2| save R3L
 ...
 || MOV32   *XAR3++, R5L        ;  | | | save R5L,                    ARP->XAR3
    ...
    ADDB    AH, #-1             ;  | | | Last instruction to change the flags
    MOV32   *0++, R0H           ;  | | | save R0H, XAR5 += 26
    MOV32   *XAR6++, R1L        ;  | | | save R1L,                    ARP->XAR6
    MOV32   *0++, R1H           ;  | | | save R1H, XAR6 += 26
    MOVZ    AR0, AL             ;  | | | XAR0 = [0:FFT size/2]
	BF    	_RFFT_adc_f64u_stages1and2and3withBitReverse_loop, NEQ
	SUBB	SP, #LF_SIZE		; Deallocate local frame
    LRETR
    .endasmfunc
    .unasg  STAGE_1_2_3_INPUT_PTR
    .unasg  STAGE_1_2_3_OUTPUT_PTR
	...
	.unasg	LF_SIZE
;;*****************************************************************************
;; void RFFT_adc_f64u_stages4andUp(void)
;;
;; Stages 4 and up
;;
;; NOTE: 
...
_RFFT_adc_f64u_stages4andUp:
    ...
    .asg    XAR7                            , COSptr
    .asg    XAR7                            , SINptr
    .asg    @_RFFTF64_U_ADC_vars_InPtr        , STAGE_4_UP_INPUT_PTR    
    .asg    @_RFFTF64_U_ADC_vars_OutPtr       , STAGE_4_UP_OUTPUT_PTR    
    .asg    @_RFFTF64_U_ADC_vars_TwiddlesPtr  , STAGE_4_UP_TWIDDLES_PTR
    .asg    @_RFFTF64_U_ADC_vars_Stages       , STAGE_4_UP_FFT_STAGES     
    .asg    @_RFFTF64_U_ADC_vars_FFTSize      , STAGE_4_UP_FFT_SIZE
...
;;-----------------------------------------------------------------------------
    MOVW    DP, #_RFFTF64_U_ADC_vars_InPtr ; Load the DP once
    MOV     AL, STAGE_4_UP_FFT_STAGES
    SUB     AL, #3                       ; If (fft->FFTStages <= 3) exit
    B       _RFFT_adc_f64u_stages4andUp_end, LEQ
...
;; Stage 4 and Up    
;;-----------------------------------------------------------------------------
_RFFT_adc_f64u_stages4andUp_stageLoop:
;;-----------------------------------------------------------------------------
;;
;; NOTE: Instead of referring to the real part as Rn we choose Dn to avoid 
;; confusing them with the Rn FPU registers
...
;;      *X4ptr = Y4 = -X4 (*X4ptr)
;;-----------------------------------------------------------------------------
_RFFT_adc_f64u_stages4andUp_outerLoop:
 ...
 || MOV32   *X3ptr++, R4L           ;  | | | 
    ...
    .align  2
    RPTB    _RFFT_adc_f64u_stages4andUp_innerLoop, InnerLoopCnt
...
_RFFT_adc_f64u_stages4andUp_innerLoop:
 ...
 || MOV32   *+X4ptr[0], R7L         ;  | | |
    ...
    BANZ    _RFFT_adc_f64u_stages4andUp_outerLoop, OuterLoopCnt--
;------------------------------------------------------------------------------
;   End of Outer Loop
...
;------------------------------------------------------------------------------
    ...
    DEC     *-SP[IDX_I]             ; IDX_I--
    BF      _RFFT_adc_f64u_stages4andUp_stageLoop, NEQ

_RFFT_adc_f64u_stages4andUp_end:
    SUBB    SP, #FRMSZ
    LRETR
    .endasmfunc
    .unasg  X3ptr
