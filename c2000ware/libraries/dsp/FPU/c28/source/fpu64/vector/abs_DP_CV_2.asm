;;#############################################################################
;;! \file source/fpu64/vector/abs_DP_CV_2.asm
;;!
...
;;
;; FUNCTIONS:
;;   void abs_DP_CV_2(long double *y, const complexf64_t *x, 
;;                  const uint16_t N)
;;  
...
;; RETURNS:    none
;;
;; BENCHMARK:  79*N/2 + 17 cycles (including the call and return)
;;
;; NOTES:
...
_abs_DP_CV_2_loop:
 ...
 || ADDF64      R0, R0, R1      ;  |   R0 = x_re[i]^2 + x_im[i]^2--------X_i
    ...
    .align  2
    RPTB        #_abs_DP_CV_2_isqrt_loop, #2 
...
_abs_DP_CV_2_isqrt_loop:
    ...
    SUBF64      R7, #1.5, R7    ; 1|*| R7 = 1.5 - Ye_i+1'''^2*X_i+1*0.5
    NOP                         ; 2|1|
    ;; R1 = Ye_i'''*(1.5 - Ye_i'''^2*X_i*0.5)
    ;; R5 = Ye_i+1'''*(1.5 - Ye_i+1'''^2*X_i+1*0.5)
    MPYF64      R1, R1, R3      ; *|2| 
    MPYF64      R5, R5, R7      ; 1|*| 
    CMPF64      R0, #0.0        ; 2|1| if X_i == 0, set ZF, NF
    MOV64       R1, R0, EQ      ;  |2| If X_i is zero, set Ye_i'''' to 0
    MPYF64      R0, R0, R1      ; *| | R0 = Y[i] = X_i * Ye_i''''
    CMPF64      R4, #0.0        ; 1| | if X_i+1 == 0, set ZF, NF
    MOV64       R5, R4, EQ      ; 2| | If X_i+1 is zero, set Ye_i+1'''' to 0
    MPYF64      R4, R4, R5      ; *| | R4 = Y[i+1] = X_i+1 * Ye_i+1''''
    MOV32       *XAR4++, R0L    ; 1| | 
    MOV32       *XAR4++, R0H    ; 2| |
    MOV32       *XAR4++, R4L    ;  | | 
    MOV32       *XAR4++, R4H    ;  | |
    BANZ        _abs_DP_CV_2_loop, AR0--
