;;#############################################################################
;;! \file source/fpu64/vector/mac_DP_i16RVxCV.asm
;;!
;;! \brief  C-Callable multiply-and-accumulate of a signed 16-bit integer real
;;              vector and a double precision complex vector
...
;;
;; FUNCTIONS:
;;   extern complexf64_t mac_DP_i16RVxCV(const complexf64_t *w,
;;                            const int16_t *x, const uint16_t N)
;;  
;; PARAMETERS:  complexf64_t *w = input complex array
;;              int16_t *x      = input real array
;;              uint16_t N      = length of w and x arrays
;;
;; RETURNS:    complexf64_t y = complex result
;;
;; BENCHMARK:   6*N + 36 cycles if N=even (including the call and return)
;;              6*N + 29 cycles if N=odd (including the call and return)
;;
;; NOTES:
...
;;*****************************************************************************
;;  complexf64_t mac_DP_i16RVxCV(const complexf64_t *w,
;;                            const int16_t *x, const uint16_t N)
;;
...
;;*****************************************************************************
    .if __TI_EABI__
    .asg mac_DP_i16RVxCV, _mac_DP_i16RVxCV
    .endif
    
    .page
    .global _mac_DP_i16RVxCV
    .sect   ".text"
_mac_DP_i16RVxCV:
    .asmfunc
    ...
    MOV32       R0L, *XAR4++        ; 1| load first w_re[i]
    MOV32       R0H, *XAR4++        ;  | 
    F32TOF64    R1, R1H             ; 
 ...
 || MOV32       R0L, *XAR4++        ;  | | R0 = w_re[i+1]    
    ...
    BF          _mac_DP_i16RVxCV_odd, TC   
    I16TOF32    R5H, *XAR5++        ; *|   load x[i+1]
    NOP                             ; 1|   
    F32TOF64    R5, R5H             ;      
    MACF64      R3, R2, R2, R5, R0  ; *|   y_re += R2, R2 = x[i+1]*w_re[i+1]
 || MOV32       R0L, *XAR4++        ;  |   load w_im[i+1]
    MOV32       R0H, *XAR4++        ; 1|   
    MACF64      R7, R6, R6, R5, R0  ; 2|*| y_im += R6, R6 = x[i+1]*w_im[i+1]
 || MOV32       R0L, *XAR4++        ;  | | load w_re[i]
    MOV32       R0H, *XAR4++        ;  |1| 
_mac_DP_i16RVxCV_odd:
    ;; Main Loop
    I16TOF32    R1H, *XAR5++        ; *|2| load x[i]
    NOP                             ; 1|   
    F32TOF64    R1, R1H             ; 
    .align  2
    RPTB        _mac_DP_i16RVxCV_loop, AL
    MACF64      R3, R2, R2, R1, R0  ; *|   y_re += R2, R2 = x[i]*w_re[i]
 || MOV32       R0L, *XAR4++        ;  |   load w_im[i+1]
    MOV32       R0H, *XAR4++        ; 1|    
    MACF64      R7, R6, R6, R1, R0  ; 2|*| y_im += R6, R6 = x[i]*w_im[i]
 || MOV32       R0L, *XAR4++        ;  | | load w_re[i+1]
    I16TOF32    R5H, *XAR5++        ; *|1| load x[i+1]
    MOV32       R0H, *XAR4++        ; 1|2| 
    F32TOF64    R5, R5H             ;  | | 
 ...
 || MOV32       R0L, *XAR4++        ;  |   load w_im[i+1]
    ...
    MACF64      R7, R6, R6, R5, R0  ; 2|*| y_im += R6, R6 = x[i+1]*w_im[i+1]
 || MOV32       R0L, *XAR4++        ;  | | load w_re[i]
    I16TOF32    R1H, *XAR5++        ; *|1| load x[i]
    MOV32       R0H, *XAR4++        ; 1|2| 
    F32TOF64    R1, R1H             ;  | | 
_mac_DP_i16RVxCV_loop:
