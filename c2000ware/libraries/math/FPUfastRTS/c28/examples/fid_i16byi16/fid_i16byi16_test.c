//#############################################################################
//
//! \file   fid_i16byi16_test.c
//!
//! \brief  Runs the FID_i16byi16 routine
...
int16_t test_output_quo[TEST_SIZE], test_output_rem[TEST_SIZE];
int16_t test_error_quo[TEST_SIZE], test_error_rem[TEST_SIZE];
int16_t num_rem, den_quo;
//*****************************************************************************
...
void FastRTS_runTest(void)
{
    ...
    for(i = 0U; i < TEST_SIZE; i++)
    {
    	...
    	FID_i16byi16(&num_rem, &den_quo);

    	test_output_quo[i]   = den_quo;
    	test_output_rem[i]   = num_rem;

    	if(FID_Example_calcError((int16_t *)&den_quo,
    	                (int16_t *)&test_quotient[i],
						(int16_t *)&test_error_quo[i],
    	                &tolerance,
						ERRORTYPE_I16) != -1)
        {
    		pass++;
