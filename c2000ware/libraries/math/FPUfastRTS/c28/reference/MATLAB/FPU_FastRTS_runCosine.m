...
%  vector
%
%  [C,R,AE,RE] = FPU_FastRTS_runCosine(512, [-pi,pi], 'TYPE_FLOAT', 
%  'sincosTable.asm', 'cosInput.asm', 'cosOutput.asm')
% 
% or
%
%  [C,R,AE,RE] = FPU_FastRTS_runCosine(512, [-pi,pi], 'TYPE_FLOAT', 
%  'sincosTable.c', 'cosInput.c', 'cosOutput.c')
% 
function [COS,REF,AERR,RERR] = FPU_FastRTS_runCosine(N, SWP, varargin)
%% Parse Arguments
nOptArgs = 4;
...
figure; 
subplot(2,2,1), plot(t,COS,'-b',t,REF,'--r'), legend('Table Lookup Cosine',...
...
if (~strcmp(InFn,'NULL') && strcmp(ext,'.asm'))
    ...
    fprintf(fid,[';;! \\brief  Cosine Input Vector (',num2str(N),') \n']);
...
if (~strcmp(OutFn,'NULL') && strcmp(ext,'.asm'))
    ...
    fprintf(fid,[';;! \\brief  Cosine Output Vector (',num2str(N),') \n']);
    fprintf(fid,';;! \\author Vishal Coelho \n');
    fprintf(fid,[';;! \\date   ',date,'\n;;! \n;;\n']);
    fprintf(fid,';;  Group:\t\t\tC2000\n');
...
if (~strcmp(OutFn,'NULL') && strcmp(ext,'.c'))
    ...
    fprintf(fid,HEAD_string);
    fprintf(fid,['//! \\file ',OutFn,'\n']);
    fprintf(fid,['//! \\brief  Cosine Ouput Vector (',num2str(N),') \n']);
    fprintf(fid,'//! \\author Vishal Coelho \n');
    fprintf(fid,['//! \\date   ',date,'\n//! \n//\n']);
    fprintf(fid,'//  Group:\t\t\tC2000\n');
    fprintf(fid,'//  Target Family:\t$DEVICE$\n//\n');
