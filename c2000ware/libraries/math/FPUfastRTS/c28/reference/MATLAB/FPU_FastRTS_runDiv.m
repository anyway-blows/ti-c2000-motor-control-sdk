...
% \brief  generate Division Input/Ouput Vectors
...
% The algorithm begins by taking the inverse of the operand.
% Note that on the C28x this is an estimated value (EINVF32)
%
% $$Y_{e} = Estimate(\frac{1}{X}) $$
%
...
%
% Since we are attempting to find $\frac{A}{B}$, we use Newton's Iterative 
% method to approximate $\frac{1}{B}$ first and them multiply by $A$ to 
% get $\frac{A}{B}$
%
% To do this, we try to solve $\frac{1}{x} - B = 0$. Applying Newton's method 
% we get
%
% $$x_{n+1} = x_{n}(2 - B{\times}x_{n}) $$
%
%% Required Parameters
% * *N*, desired size of the lookup table
% * *SWPN* , array indicating the desired input numerator sweep, e.g. 
% [0.002, +pi] Try and keep the sweep within a decade else you will see a 
% very large step between successive values
% * *SWPD* , array indicating the desired input denominator sweep, e.g. 
% [-pi, 0.002] Try and keep the sweep within a decade else you will see a 
% very large step between successive values.
%
%% Optional Parameters
...
% *REF*,  the reference Qutotient (calculated using matlab in-built
% functions)
% *AERR*, Absolute error
% *RERR*, Relative error
...
%  vector
%
%  [Q,R,AE,RE] = FPU_FastRTS_runDiv(512, [1,10], [-14,32], 'TYPE_FLOAT',
%  'divInput.asm', 'divOutput.asm')
% 
% or
%
%  [Q,R,AE,RE] = FPU_FastRTS_runDiv(512, [exp(1), pi], [-2, exp(1)], 
%  'TYPE_FLOAT', 'divInput.c', 'divOutput.c')
% 
function [QUO,REF,AERR,RERR] = FPU_FastRTS_runDiv(N, SWPN, SWPD, varargin)
%% Parse Arguments
nOptArgs = 3;
...
j = 1;
for i = 1 : N
    % Take an estimate of 1/denominator. Now since this is 
...
if (strcmp(precision,'TYPE_DOUBLE'))    
    ...
    QUO(i) = xn * num;
    REF(i) = num/den;
    AERR(i) = abs(REF(i) - QUO(i));
    RERR(i) = AERR(i)/max(abs(num), abs(REF(i)));
    IN(j)   = num;
    IN(j+1) = den;
    j = j + 2;
    num  = num + stepn;
...
figure; 
t = linspace(1,N, N);
subplot(2,2,1), plot(t,QUO,'-b',t,REF,'--r'),...
    legend('Newton-Raphson Division','Reference MATLAB Division'),...
    xlabel('theta'), ylabel('Amplitude'),...
    title('Division'), grid on;
...
if (~strcmp(InFn,'NULL') && strcmp(ext,'.asm'))
    ...
    fprintf(fid,[';;! \\brief  Division Input Vectors (',num2str(N),') \n']);
...
if (~strcmp(OutFn,'NULL') && strcmp(ext,'.asm'))
    ...
    fprintf(fid,[';;! \\brief  Division Output Vector (',num2str(N),') \n']);
    fprintf(fid,';;! \\author Vishal Coelho \n');
    fprintf(fid,[';;! \\date   ',date,'\n;;! \n;;\n']);
    fprintf(fid,';;  Group:\t\t\tC2000\n');
...
if (~strcmp(OutFn,'NULL') && strcmp(ext,'.c'))
    ...
    fprintf(fid,HEAD_string);
    fprintf(fid,['//! \\file ',OutFn,'\n']);
    fprintf(fid,['//! \\brief  Division Ouput Vector (',num2str(N),') \n']);
    fprintf(fid,'//! \\author Vishal Coelho \n');
    fprintf(fid,['//! \\date   ',date,'\n//! \n//\n']);
    fprintf(fid,'//  Group:\t\t\tC2000\n');
    fprintf(fid,'//  Target Family:\t$DEVICE$\n//\n');
    ...
    if(strcmp(precision, 'TYPE_FLOAT'))
        ...
        S = sprintf('\t%15.12e, %15.12e, %15.12e, \n',QUO);
    end
    fprintf(fid,S);
    fprintf(fid,'}; \n\n');
    fprintf(fid,'\n// End of File\n');
