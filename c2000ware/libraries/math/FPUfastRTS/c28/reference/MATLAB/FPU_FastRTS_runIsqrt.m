...
% \brief  generate Inverse Square Root Input/Ouput Vectors
...
%% The Inverse Square Root Algorithm
...
%
% We try to solve $\frac{1}{x^{2}} - S = 0$. The advantage of trying to solve 
% this equation over $x^{2}-\frac{1}{S}=0$ is that we dont end up 
% having to do any division. 
%
...
%
%% Returns
% *ISQRT*, the calculated Inverse Square Root
...
%  vector
%
%  [I,R,AE,RE] = FPU_FastRTS_runIsqrt(512, [1,10], 'TYPE_FLOAT',
%  'isqrtInput.asm', 'isqrtOutput.asm')
% 
% or
%
%  [I,R,AE,RE] = FPU_FastRTS_runIsqrt(512, [exp(1), pi], 'TYPE_FLOAT',
%  'isqrtInput.c', 'isqrtOutput.c')
% 
function [ISQRT,REF,AERR,RERR] = FPU_FastRTS_runIsqrt(N, SWP, varargin)
%% Parse Arguments
nOptArgs = 3;
...
if (strcmp(precision,'TYPE_DOUBLE'))    
    ...
    ISQRT(i) = xn;
    REF(i) = 1/sqrt(op);
    AERR(i) = abs(REF(i) - ISQRT(i));
    RERR(i) = AERR(i)/max(abs(op), abs(REF(i)));
    t(i)  = op;
    op  = op + step;
end
figure; 
subplot(2,2,1), plot(t,ISQRT,'-b',t,REF,'--r'),...
    legend('Newton-Raphson Inverse Square Root',...
    'Reference MATLAB Inverse Square Root'),...
    xlabel('theta'), ylabel('Amplitude'),...
    title('Inverse  Square Root'), grid on;
...
if (~strcmp(InFn,'NULL') && strcmp(ext,'.asm'))
    ...
    fprintf(fid,[';;! \\brief  Inverse Square Root Input Vector (',...
        num2str(N),') \n']);
...
if (~strcmp(OutFn,'NULL') && strcmp(ext,'.asm'))
    ...
    fprintf(fid,[';;! \\brief Inverse Square Root Output Vector (',...
        num2str(N),') \n']);
    fprintf(fid,';;! \\author Vishal Coelho \n');
    fprintf(fid,[';;! \\date   ',date,'\n;;! \n;;\n']);
    fprintf(fid,';;  Group:\t\t\tC2000\n');
...
if (~strcmp(OutFn,'NULL') && strcmp(ext,'.c'))
    ...
    fprintf(fid,HEAD_string);
    fprintf(fid,['//! \\file ',OutFn,'\n']);
    fprintf(fid,['//! \\brief Inverse Square Root Ouput Vector (',...
        num2str(N),') \n']);
    fprintf(fid,'//! \\author Vishal Coelho \n');
    fprintf(fid,['//! \\date   ',date,'\n//! \n//\n']);
    fprintf(fid,'//  Group:\t\t\tC2000\n');
