;;#############################################################################
;;! \file source/fpu64/cordic_sin_f64.asm
;;!
;;! \brief  CORDIC sin function for the C28x + FPU64
...
;; FUNCTIONS:
;;
;;  float64u_t CORDIC_F64_sin(hnd_CORDIC_t hC, float64u_t theta)
;;  
...
;;          0
;;  3. The y  is the sine of the angle. Convert this angle back to float
;;          i
;;     and return
;;
;;  Group:            C2000
...
;; external references
    ...
    .asg    CORDIC_F64_sin, _CORDIC_F64_sin
    .endif
...
_CORDIC_F64_sin_convertArg:
    ...
    LCR         *KERNELPTR
                                        ;    XAR6 -> y_i (IQ61)
    MOV32       R0L, *+Y_I[0]           ;    R0 = IQ61(sin(theta))
    MOV32       R0H, *+Y_I[2]
    I64TOF64    R0, R0                  ; *| R0 = sin(theta) * 2^61
    NOP                                 ; 1| 
