...
TENOR_CLB_HLC::TENOR_CLB_HLC(sc_core::sc_module_name _nm,
...
void TENOR_CLB_HLC::hlc_mem_init(void) {
...
void TENOR_CLB_HLC::hlc_logic(void) {
  int i;
...
// sc_uint<5> hlc_instr_rd_ptr; 
// sc_uint<12> hlc_instr_rd_val; 
//  sc_uint<8> clb_result_in_S;
...
// hlc_instr_rd_ptr = HLC_INSTR_READ_PTR.read();
// hlc_instr_rd_val = HLC_INSTR_READ_VALUE.read();
   ...
   if (rst_n == 0) {
    ...
    for (i = 0; i<=7; i=i+1) {
//			output_event_wire_local_reg_7_0[i] = ((RECONFIG_PIPELINE_EN == 1) ?  clb_result_in_REG[i] : clb_result_in[i].read());
//			output_event_wire_local_reg_15_8[i] = ((RECONFIG_PIPELINE_EN == 1) ?  ~clb_result_in_REG[i] : ~clb_result_in[i].read());
			if(RECONFIG_PIPELINE_EN == 1) {
			output_event_wire_local_reg_7_0[i] = clb_result_in_REG[i];
			output_event_wire_local_reg_15_8[i] = ~clb_result_in_REG[i];
			} else {
...
// Brute force method to detect event being serviced and correspoding EVET register getting cleared
 ...
 pull_data_next_cycle = ((instr_active_T == 1) && (instr_opcode == OPCODE_PULL) && (dummy_state_T == 0)) ? 1 : 0;
...
/// Otherwise, it will spin endlessly there.
	next_cycle_dummy_state = (push_data_en || pull_data_en);
...
/////////// On an event, jump to the vector and execute till the last instruction (seen by the stop-bit set).
	next_instr_PTR = (((instr_active_T == 1) && (stop_bit == 0) && (next_cycle_dummy_state == 0)) || ((dummy_state_T == 1) && (stop_bit == 0))) ? current_instr_PTR + 1 : // Current execution is highest priority
                                    (activity_trigger && (dummy_state_T == 0) && (next_cycle_dummy_state == 0)) ? event_instr_ptr :   // New trigger
				    current_instr_PTR;

    instr_active_next_state = ((activity_trigger && (dummy_state_T == 0) && (next_cycle_dummy_state == 0))
                                            || ((instr_active_T == 1) && (stop_bit == 0) && (next_cycle_dummy_state == 0))
                                            || ( (dummy_state_T == 1) && (stop_bit == 0)) // If this is a dummy_state, then next state has to be active.
								);
...
// hlc_instr_rd_val = INSTR_ARRAY[hlc_instr_rd_ptr];
   ...
   for (i=0; i<=7; i++) {
   clb_result_in_REG[i] = clb_result_in[i].read();		//1 cycle delay will to be introduced here for the pipelined version of the signal
	}       
