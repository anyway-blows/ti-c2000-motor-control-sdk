...
void TENOR_CLB_COUNTER::counter_logic()
{
	...
	RECONFIG_PIPELINE_EN_S = RECONFIG_PIPELINE_EN.read();
    ...
    counter_prev_REG 		= counter_prev_REG_SIG;
    counter_prev_REG_SIG 	= counter_REG;
    ...
    if (RECONFIG_PIPELINE_EN_S == 0)
    {
   	...
   	{
	    load_val = ((add_shift_mode_S == 1) && (add_shift_dir_S == 1))? (counter_prev_REG + event_load_val_S ) :
			       ((add_shift_mode_S == 1) && (add_shift_dir_S == 0))? (counter_prev_REG - event_load_val_S) :
			       ((add_shift_mode_S == 0) && (add_shift_dir_S == 1))? (counter_prev_REG << (event_load_val_S & 0x1f) ) :
			       ((add_shift_mode_S == 0) && (add_shift_dir_S == 0))? (counter_prev_REG >> (event_load_val_S & 0x1f) ) : 0;
   	}
